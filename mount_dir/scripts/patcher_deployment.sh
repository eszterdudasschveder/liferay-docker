# This script creates another script under /usr/local/liferay/scripts/pre-startup
# which deploys osb-patcher-portlet before startup.
mkdir -p /usr/local/liferay/scripts/pre-startup
cat << EOF > /usr/local/liferay/scripts/pre-startup/0_deploy_patcher.sh
#!/bin/bash
...
chmod u+x /usr/local/liferay/scripts/pre-startup/0_deploy_patcher.sh