...
function make_temp_directory {
	...
	echo $TEMP_DIR temp directory created
}
...
function update_properties_files {
	sed -i "s/localhost:3306/mysql:3306/g" ${TEMP_DIR}/liferay/portal-ext.properties
	sed -i "s/localhost:3306/mysql:3306/g" ${TEMP_DIR}/liferay/portal-setup-wizard.properties
}

function update_path_and_java_home {
